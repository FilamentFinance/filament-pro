...
# Remove modules # && git add . && git commit -m "modules"
remove :; rm -rf .gitmodules && rm -rf .git/modules/* && rm -rf lib && touch .gitmodules 

# && forge install dk1a/solidity-stringutils --no-commit
install :; forge install foundry-rs/forge-std --no-commit && forge install OpenZeppelin/openzeppelin-foundry-upgrades --no-commit 
