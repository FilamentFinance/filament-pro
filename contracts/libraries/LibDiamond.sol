...
library LibDiamond {
    ...
    // event OwnershipTransferreds(address indexed previousOwner, address indexed newOwner);

    function setContractOwner(address _newOwner) internal {
        DiamondStorage storage ds = diamondStorage();
        // address previousOwner = ds.contractOwner;
        ds.contractOwner = _newOwner;
        // emit OwnershipTransferreds(previousOwner, _newOwner);
    }
    ...
    function addReplaceRemoveFacetSelectors(
        ...
        if (_action == IDiamondCut.FacetCutAction.Add) {
            ...
            for (uint256 selectorIndex; selectorIndex < _selectors.length;) {
                ...
                // @audit - PVE001 - Fixed
                require(
                    address(bytes20(oldFacet)) == address(0) && selector != bytes4(0), "LibDiamondCut: Can't add function that already exists"
                );
            ...
            for (uint256 selectorIndex; selectorIndex < _selectors.length;) {
                ...
                // adding a block here prevents stack too deep error
                {
                    ...
                    uint256 oldSelectorCount = uint96(uint256(oldFacet));
